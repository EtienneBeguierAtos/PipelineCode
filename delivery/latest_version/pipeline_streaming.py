...
from pipeline_commons import custom_read_file
...
def read_csv(message):
    with beam.io.gcsio.GcsIO().open(filename=message["fileURL"], mode="r") as f:
        ...
        file_data=f.read().decode(file_struct['encoding'])
        logging.warning('Data processed: '+file_name)
        return {"data":file_data, "message": message, "file_struct":file_struct,"source_file":file_name}

def split_file(element):
    #embed()
    file_struct=element["file_struct"]
    file_lines=element["data"].split(file_struct["delimiter"])
    ...
    logging.warning('Lines splitted: '+element["source_file"])
    return {"data":file_lines,"message":element["message"],"schema":schema,"source_file":element["source_file"]}


def separate_lines(element):
    lines_list=map(lambda line:{
        "data":line,
...
def mock_read_line(element):
    schema=STRUCTURE_SOURCE["telephone"]["Schema"]
    splitted_line=element.split(",")
    output_row={
        "is_line":True,
        "stacktrace":"",
        "source_file":"telephone4.csv",
        "raw_data":"raw",
        "stacktrace":"fake",
...
def run_streaming():
    ...
    commons.OPTIONS.view_as(StandardOptions).streaming=True

    p = beam.Pipeline(options=commons.OPTIONS)
    ...
    data=(read_messages
            #| 'GetURL' >> beam.Map(lambda element: element["fileURL"])
            | 'Read CSV files' >> custom_read_file.custom_ReadAllFromText(skip_header_lines=parameters["skipLeadingRows"]) #beam.io.ReadAllFromText(skip_header_lines=parameters["skipLeadingRows"])
    ...
    (data 
            ...
            | 'WriteWithDynamicDestination' >> beam.io.WriteToBigQuery(
                table=commons.OPTIONS.view_as(GoogleCloudOptions).project + ":" + DATASET + "."+ "telephone", #get_destination_table,
                schema=get_schema,
                ignore_unknown_columns=True,
                #additional_bq_parameters={'ignoreUnknownValues': True,'maxBadRecords': 1000},
                create_disposition=beam.io.BigQueryDisposition.CREATE_NEVER,
