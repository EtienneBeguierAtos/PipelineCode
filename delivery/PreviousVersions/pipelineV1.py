...
class ReadCsv(beam.DoFn):
    ...
    def process(self, message):
        ...
        bucket = Storage_Client.get_bucket(address[2])
...
class InsertCsv(beam.DoFn):
    def start_bundle(self):
        from google.cloud import bigquery
        #logging.warning('Creation BigQuery client!')
    ...
    def process(self, pair):
        ...
        tables = Bigquery_Client.list_tables(Options.view_as(GoogleCloudOptions).project + "." + dataset)
        list_id=[]
        for table in tables:
        ...
        Bigquery_Client.insert_rows_json(table_id, dicts)
