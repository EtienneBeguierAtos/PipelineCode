...
def check_partition(element):
    row=element[1]
    if "partition_depth" in row:
        depth=row["partition_depth"]
        key=row["partition_key"]
        if depth[0]>=row[key] or depth[1]<=row[key]:
            row["stacktrace"]="not in queried partition"
    return (element[0], row)
